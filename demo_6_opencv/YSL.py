...
def coloring(imgfile):
    ...
    for aim in aim_list:
        ...
        if ret:
            ...
            cv2.putText(AddText, aim, (100, 100), cv2.FONT_HERSHEY_PLAIN, 4.0, (85, 37, 209), 5)
            np.hstack([img, AddText])
            res = np.hstack([res_img, AddText])
            cv2.waitKey(1)
            print(imgfile.split('pic1/')[1].split('_')[0], aim)
            filename = "D:/github/1/hotpoor_autoclick_xhs/demo_6_opencv/test/YSL/%s_%s.jpg" % (imgfile.split('pic1/')[1].split('_')[0], aim)
