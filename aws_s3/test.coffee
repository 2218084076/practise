...
<div class="upload_status_str">请选择图片</div>
<input type="file" class="aws_file_input">
<button class="aws_file_add">upload</button>
<img class="demo" src="">
...
$("body").on "click",".aws_file_add",(evt)->
    time_start = (new Date()).getTime()
    $(".upload_status_str").text "上传中"
    file = $(".aws_file_input")[0].files[0]
    params =
    ...
    bucket.upload params,(err,data)->
        ...
        cost_str = "cost:"+(time_end-time_start)/1000.0
        $(".upload_status_str").text "上传完成，耗时#{cost_str}秒，可选择新图片"
        $(".comments_area[data-block=5749e18180b040bdad8f7eef879389d3]").find(".comment_content").val(JSON.stringify(data))
        $(".comments_area[data-block=5749e18180b040bdad8f7eef879389d3]").find(".comment_submit").click()
...
time_start = (new Date()).getTime()
console.log()
bucket.getObject {Key: "bangfer/0b55b319ebc4b7453372eebb9fbacf138b821507.jpeg"},(err,file)->
    console.log err
    console.log file
    file_type = file.ContentType
    img_src = "data:#{file_type};base64," + encode_from_s3(file.Body)
    $("img.demo").attr "src",img_src
    time_end = (new Date()).getTime()
    console.log "cost:",(time_end-time_start)/1000.0
...
$("body").on "click",".load_file",(evt)->
    ...
    bucket.getObject {Key: dom_load_key},(err,file)->
        ...
        if file_type.split("/")[0] in ["image"]
            ...
            """
        else if file_type.split("/")[0] in ["video"]
            dom_preview.append """
                <video src="#{file_src}" style="width:100%;" controls></video>
            """
        else
            dom_preview.append """
        ...
        time_end = (new Date()).getTime()
        cost_str = "cost:"+(time_end-time_start)/1000.0
        dom_preview_str.text "加载完毕，耗时#{cost_str}秒"
